...
Escudero. nesse caso usamos 2 temperaturas para as espécimes de neutrinos.
'''
...
'''
Tve = temperatura dos neutrinos dos elétrons
Tvmu = temperatura dos neutrinos do múon
Te = temperatura dos elétrons
Tg = temperatura do fótons (Te = Tg para nós)

As densidades a seguir se referem a energia:

pve = densidade de energia dos neutrinos dos elétrons
pvmu = densidade de energia dos neutrinos dos múons
...
def pT(Tg,Tve,Tvmu):
    return pe(Tg)+ pg(Tg)+ pve(Tve) + 2*pvmu(Tvmu)
...
def func1(y,t):
    Tve, Tvmu, Tg = y
    return - H(pe(Tg),pve(Tve),pg(Tg),pvmu(Tvmu))*Tve + (dpve_dt(Tg, Tve, Tvmu) / dpve_dT(Tve))

def func2(y,t):
    Tve, Tvmu, Tg = y
    return - H(pe(Tg),pve(Tve),pg(Tg),pvmu(Tvmu))* Tvmu  +  dpvmu_dt(Tg,Tve,Tvmu)/ dpvmu_dT(Tvmu)

def func3(y,t):
    Tve, Tvmu, Tg = y
    h = H(pe(Tg),pve(Tve),pg(Tg),pvmu(Tvmu))
    return -(4 *h* pg(Tg)+3 * h * (pe(Tg)+ Pe(Tg)) + dpve_dt(Tg, Tve, Tvmu) + 2 * dpvmu_dt(Tg, Tve, Tvmu) ) / (dpg_dT(Tg) + dpe_dT(Tg))

def Func(y,t):
    Tve , Tvmu , Tg = y
    dy_dt = np.array([func1(y,t),func2(y,t),func3(y,t)])
    return dy_dt
...
def main():
    ...
    plt.plot(sol[:,2], sol[:, 2]/ sol[:, 0], 'b', label='T\u03B3/T\u03BDe', color = 'k')
    plt.plot(sol[:,2], sol[:, 2]/ sol[:, 1], 'b', label='T\u03B3/T\u03BD\u03BC', color = 'b')
    plt.legend(loc='best')
    plt.xlabel('T\u03B3 [Mev]')
    #plt.grid()
    ...
    print('$N_{eff}$ = ', (8/7) *(11/4)**(4/3) * ((pve(sol[:,0][-1])+2*pvmu(sol[:,1][-1])+pe(sol[:,2][-1]))/pg(sol[:,2][-1])))
    '''
    print('$N_{eff}$ = ', (8/7) *(11/4)**(4/3) * ((pv(sol[:,0][-1])-pg(sol[:,1][-1]))/pg(sol[:,1][-1])))
    '''
    #print('$N_{eff}$ = ', 3 *(11/4)**(4/3) * ((sol[:,0][-1])/sol[:,2][-1])**4)
    print('Tve/TVmu =',sol[:, 0][-1]/sol[:, 1][-1])
